...
test("Test to see if App renders", () => {
    mockFetchShow.mockResolvedValueOnce(infoData);
    render(<App/>);

})

test('Updates episodes list on dropdown change', async () => {
    mockFetchShow.mockResolvedValue(infoData)
    const { getByText, getAllByText, queryAllByTestId, queryByText } = render(<App />)
